...
export const DEFAULT_SETTINGS: AppSettings = {
  ...
  visibleTools: ['qr', 'encryption', 'to-base64', 'bg-remover', 'auto-passport', 'video', 'dex-protector', 'lib-encryptor', 'apk-store', 'dih-movies', 'temp-mail', 'mobile-bypass', 'hosted-admin', 'reels-maker'],
  newTools: ['qr', 'encryption', 'to-base64', 'bg-remover', 'auto-passport', 'video', 'dex-protector', 'lib-encryptor', 'apk-store', 'dih-movies', 'temp-mail', 'mobile-bypass', 'hosted-admin', 'reels-maker'],
  newBadgeText: 'NEW',
  toolLabels: {
    'qr': 'QR Code Tools',
  ...
  toolDescriptions: {
    ...
    'hosted-admin': 'Host and manage your own custom HTML/CSS/JS templates.',
    'reels-maker': 'Export gorgeous high-tech promotion vertical reels for Reels and TikTok.'
  },
  ...
  liveUserRange: 450,
  defaultQRUrl: 'https://diddyhub.pro',
  paybdApiKey: '',
...
export function useAppSettings() {
  const [settings, setSettings] = useState<AppSettings>(() => {
    try {
      ...
      const healedVisibleTools = [...new Set([...parsedVisibleTools, ...DEFAULT_SETTINGS.visibleTools])]
        .filter(id => id !== 'nid' && id !== 'speed-test' && id !== 'daily-compound' && id !== 'temp-sms' && id !== 'passport' && id !== 'design-editor' && id !== 'cut-downloader');
      ...
      return {
        ...DEFAULT_SETTINGS,
        ...parsed,
        primaryColor: DEFAULT_SETTINGS.primaryColor, // Force latest color
        toolLabels: { ...DEFAULT_SETTINGS.toolLabels }, // Force latest labels
        toolDescriptions: { ...DEFAULT_SETTINGS.toolDescriptions }, // Force latest descriptions
        appName: DEFAULT_SETTINGS.appName, // Force app name
        footerText: DEFAULT_SETTINGS.footerText, // Force footer text
        dashboardStats: DEFAULT_SETTINGS.dashboardStats, // Force latest stats labels/values
        templates: mergedTemplates,
        visibleTools: healedVisibleTools,
        upcomingTools: parsed.upcomingTools || [],
        comingSoonTools: parsed.comingSoonTools || []
      };
    } catch (e) {
  ...
  return { settings, updateSettings, addTemplate, removeTemplate };
}
