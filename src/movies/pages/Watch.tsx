...
export function MoviesWatch({ movieId, onNavigate }: { movieId: string; onNavigate: (path: string) => void }) {
  ...
  return (
    <div ref={containerRef} style={{ position:'fixed', inset:0, background:'#000', display:'flex', flexDirection:'column', zIndex:9999 }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0, height:48, padding:'0 12px', background:'rgba(7,9,15,0.97)', borderBottom:'1px solid rgba(245,158,11,0.12)' }}>
        {!isFullscreen && <button onClick={() => onNavigate(`/movie/${movieId}`)} style={{ display:'flex', alignItems:'center', gap:4, padding:'6px 12px', borderRadius:7, border:'none', cursor:'pointer', background:'rgba(255,255,255,0.07)', color:'rgba(255,255,255,0.6)', fontSize:13, fontWeight:600 }}><ChevronLeft size={15}/>Back</button>}
      ...
      <div style={{ flex:1, position:'relative', background:'#000' }}>
        ...
        <div style={{ position:'absolute', top:8, right:0, width:100, height:34, background:'#000', zIndex:30, pointerEvents:'none', display:'flex', alignItems:'center', justifyContent:'flex-end', paddingRight:12 }}>
          <span style={{ fontSize:10, fontWeight:900, color:'rgba(255,255,255,0.85)' }}>DIH<span style={{ color:'#F59E0B' }}>TEMPLATE</span></span>
        </div>
      </div>
    </div>
  );
