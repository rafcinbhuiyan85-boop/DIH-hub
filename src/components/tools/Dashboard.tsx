...
import { 
  ...
  FileArchive, RefreshCcw, Globe, Server
} from 'lucide-react';
...
const tools = [
  ...
  { id: 'hosted-admin', icon: Globe, label: 'DIH TEMPLATE', description: 'Experience the infinite template gallery and hosting.', color: 'bg-orange-500', isNew: true },
  { id: 'reels-maker', icon: Film, label: 'Promo Reels Maker', description: 'Export vertical promo reels with custom titles & high-tech themes.', color: 'bg-pink-500', isNew: true },
];
...
export default function Dashboard({ onSelectTool }: DashboardProps) {
  ...
  const unfilteredVisibleTools = tools.filter(t => {
    // If tool is enabled globally via its specific toggle, show it even if visibleTools list is stale
    if (t.id === 'reels-maker') return true; // Force-enable reels-maker tool!
              ...
              <div className="relative z-10 w-full mb-auto flex flex-col items-start">
                <div className={cn(
                  ...
                  {settings.disabledTools?.includes(tool.id) ? (
                    ...
                    <div className="absolute -top-2 -right-2 badge-new px-1.5 py-0.5 rounded-full z-20">
                      <span className="text-[7px] font-black text-white uppercase leading-none tracking-tighter">
