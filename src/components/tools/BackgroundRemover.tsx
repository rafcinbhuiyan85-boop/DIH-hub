      ...
      if (useAI) {
        ...
        const response = await ai.models.generateContent({
          ...
          contents: {
            parts: [
              ...
              {
                text: "CRITICAL: Remove the background. KEEP ONLY the main subject. Replace the entire background with a SOLID, FLAT, PURE MAGENTA (#FF00FF). DO NOT add shadows, glows, or any effects around the subject. High precision subject isolation is required.",
              },
            ],
          },
        });
        ...
        if (candidates && candidates[0]?.content?.parts) {
          for (const part of candidates[0].content.parts) {
            if (part.inlineData) {
              const resData = `data:${part.inlineData.mimeType};base64,${part.inlineData.data}`;
