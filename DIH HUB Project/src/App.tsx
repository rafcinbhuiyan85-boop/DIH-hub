...
import PromoReels from './components/tools/PromoReels';
import UnderMaintenance from './components/tools/UnderMaintenance';

type ToolId = 'dashboard' | 'qr' | 'encryption' | 'to-base64' | 'img-to-base64' | 'bg-remover' | 'passport' | 'auto-passport' | 'video' | 'design-editor' | 'admin-login' | 'admin-panel' | 'lib-encryptor' | 'dex-protector' | 'apk-store' | 'temp-mail' | 'temp-sms' | 'cut-downloader' | 'mobile-bypass' | 'migration' | 'dih-movies' | 'hosted-admin' | 'reels-maker';
...
function MainApp() {
  ...
  const renderTool = () => {
    ...
    switch (activeTool) {
      ...
      case 'reels-maker': return <PromoReels />;
