...
import { Sun, Moon, LayoutDashboard, QrCode, ShieldCheck, Image as ImageIcon, UserSquare2, Download, Palette, Menu, X, ShieldAlert, Cpu, ShieldAlert as Lock, Package, Film, Mail, MessageSquare, Scissors, Star, Users, Smartphone, RefreshCcw, Globe, Server, Instagram, User, LogIn, LogOut } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { cn } from '@/src/lib/utils';
import { useAppSettings } from '@/src/hooks/useAppSettings';
import DihLogo from './DihLogo';

type ToolId = 'dashboard' | 'qr' | 'encryption' | 'to-base64' | 'img-to-base64' | 'bg-remover' | 'passport' | 'auto-passport' | 'video' | 'cut-downloader' | 'design-editor' | 'admin-login' | 'admin-panel' | 'lib-encryptor' | 'dex-protector' | 'apk-store' | 'dih-movies' | 'temp-mail' | 'temp-sms' | 'mobile-bypass' | 'hosted-admin';
...
export default function Layout({ 
  ...
  useEffect(() => {
    if (isFullScreenMode || window.innerWidth < 768) {
      setIsSidebarOpen(false);
    } else {
      setIsSidebarOpen(true);
    }
  }, [isFullScreenMode, activeTool]);
  ...
  return (
    <div className="min-h-screen flex text-slate-900 dark:text-slate-100 selection:bg-primary/30 overflow-x-hidden relative">
      ...
      <aside className={cn(
        ...
        (isFullScreenMode || !isSidebarOpen) ? "-translate-x-full md:-translate-x-full" : "translate-x-0",
        (isFullScreenMode || !isSidebarOpen) && "md:w-0",
              ...
              return (
                <React.Fragment key={item.id}>
                  ...
                  >
                    ...
                    ) : settings.newTools?.includes(item.id) && !isActive ? (
                      <span className="badge-new text-white text-[8px] px-1.5 py-0.5 rounded-full font-black">
                        {settings.newBadgeText || 'NEW'}
                      </span>
                    ) : null}
      ...
      <main className={cn(
        ...
        (!isFullScreenMode && isSidebarOpen) ? "md:ml-56" : "ml-0",
        settings.enableGlassmorphism && "backdrop-blur-3xl"
      )}>
        {!isFullScreenMode && (
        <header className={cn(
        ...
        <div className={cn(
          ...
          {!isFullScreenMode && activeTool !== 'admin-panel' && activeTool !== 'admin-login' && (
